...
def BinarySearchIns(ListIn, ItemIn):
    ...
    if len(ListIn) == 0:  # empty list, just add the item
        ...
        if Rindex - Lindex == 1:  # found an adjacent pair of items between which we can insert the input item
            ListIn.insert(Rindex,ItemIn)
        else:
            # right and left indices are concurrent.  This can happen when ItemIn has an identical time to one of the items in ListIn.  These are either the same object (in which case we don't insert), or not.
            if not ItemIn[0] is ListIn[Rindex][0]: # there can be two different simplices with exactly the same time (degenerate triangles spanning the domain), this compares the two simplices and adds them if they are different
                ListIn.insert(Rindex,ItemIn)
...
def IsIntersection(Line1,Line2,timeinfo = False):
    ...
    # determinant.  If == 0, then the lines are parallel, and there is no intersection to find
    det = D2y*D1x - D1y*D2x
    t1, t2 = 0, 0
    if not det == 0:  #lines intersect
        ...
        if not (t1 > 1 or t1 < 0):  # The intersection is between the endpoints of line 1, so look for intersection time for line 2 (t2)
            if abs(D2x) > abs(D2y):
            ...
            if not (t2 > 1 or t2 < 0):
                IsInt = True
    if timeinfo:
        return [IsInt,t1]  # return whether there is a good intersection, and the time
    else:
...
def AreaZeroTimeBaseSingle(aix,aiy,bix,biy,cix,ciy,afx,afy,bfx,bfy,cfx,cfy, Tin = 0):
    ...
    """
    #Some intermediate variables to help cut down the number of algebraic operations needed
    caix = cix-aix
    caiy = ciy-aiy
    baix = bix-aix
    baiy = biy-aiy
    ...
    bafix = bfx-afx-baix
    bafiy = bfy-afy-baiy
    #these are the three coefficients for the quadratic equation: a*t**2 + b*t + c
    ...
    if a == 0:
        ...
        if q >= 0:  # for a != 0 and non-negative discriminant there are two real roots (possibly degenerate).
            t1 = (-b-math.copysign(1,b)*math.sqrt(q))/(2*a) #the largest magnitude root to help with numerical stability (no catastrophic cancellation)
            t2 = c/(a*t1)  # the other root using t1*t2 = c/a
            t1ok, t2ok = False, False
            # checking to see if either of the roots is "good", i.e. between the bounds Tin and 1.
            if t1 < 1 and t1 > Tin:
                t1ok = True
            if t2 < 1 and t2 > Tin:
                t2ok = True 
            ...
            if (not t1ok) and (not t2ok):
                ...
                return [True,min(t1,t2)] # both times are in the interval, returning smallest time.
            elif t1ok:
...
def AreaZeroTimeBaseVec(aix,aiy,bix,biy,cix,ciy,afx,afy,bfx,bfy,cfx,cfy, Tin = 0):
    ...
    t2ok = (roots[1] > Tin) & (roots[1] < 1)  # same for second root (note that roots was initialized with a value out of the bounds)
    GoodSoln = t1ok | t2ok  # at least one good solution
    BothGood = t1ok & t2ok  # both solutions are good - need to choose smallest
    Onlyt2Good = ~t1ok & t2ok  # only second solution is good - case where we need to put solution 2 in the place of 1
    roots[0,BothGood] = np.minimum(roots[0,BothGood], roots[1,BothGood])  # smallest solution in the case of 2 good ones
    roots[0,Onlyt2Good] = roots[1,Onlyt2Good]  # putting solution 2 in the place of 1 when only 2 is a good solution
    return GoodSoln, roots[0,:]  # outputting vector of good solutions and their times
...
# IsDelaunayBaseWMask is a vectorized (+jit) function which returns a True/False array corresponding to whether the input quadrilateral (points A, B, C, D in ccw order, and diagonal edge A-C) is Delaunay
@jit(nopython=True)
...
def BezierQuad(P1, P2, P3):
    ...
    3 input points
    """
    return mpatches.PathPatch(mpath.Path([(P1[0], P1[1]), (P2[0], P2[1]), (P3[0], P3[1])], [mpath.Path.MOVETO,mpath.Path.CURVE3, mpath.Path.CURVE3]))
...
def BezierCubic(P1, P2, P3, P4):
    ...
    4 input points
    """
    return mpatches.PathPatch(mpath.Path([(P1[0], P1[1]), (P2[0], P2[1]), (P3[0], P3[1]),(P4[0], P4[1])], [mpath.Path.MOVETO,mpath.Path.CURVE4, mpath.Path.CURVE4,mpath.Path.CURVE4]))
...
def BezierCustom(P1, P2, P3, P4, P5):
    ...
    """
    return mpatches.PathPatch(mpath.Path([(P1[0], P1[1]), (P2[0], P2[1]), (P3[0], P3[1]),(P4[0], P4[1]),(P5[0], P5[1])], [mpath.Path.MOVETO,mpath.Path.LINETO, mpath.Path.CURVE3,mpath.Path.CURVE3,mpath.Path.LINETO]))